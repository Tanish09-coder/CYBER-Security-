...
import { OptimizerResponse } from '../types/risk';
...
export const InvestmentOptimizer: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await riskApi.optimizeBudget({
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-full space-y-4">
      ...
      <p className="text-sm font-medium text-text-secondary">Calculating ROI / ROSI metrics...</p>
    </div>
  );
  ...
  if (error) return (
    <div className="p-6 h-full flex flex-col items-center justify-center">
      <div className="bg-red-50 border border-red-200 p-6 rounded-lg max-w-lg text-center">
        ...
        <h3 className="text-sm font-bold text-red-900 mb-2">Optimizer Blocked</h3>
  ...
  return (
    <div className="p-6">
      <h2 className="text-lg font-bold text-text-primary mb-6">Investment Optimizer (ROSI)</h2>
      {data && (
        <div className="bg-app-surface border border-app-border p-4 rounded text-sm text-text-primary">
          Available Budget: {data.available_budget}
        </div>
      )}
    </div>
  );
